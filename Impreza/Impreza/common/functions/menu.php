...
class US_Walker_Nav_Menu extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if ( ! empty( $item->mega_menu_cols ) ) {
			$class_arr[] = 'has_cols fill_direction_' . $item->columns_fill_direction;
			$style = ' style="--menu-cols:' . $item->mega_menu_cols . '"';
		}
		...
		// Extra class when Reusable Block has rows and columns
		...
		$class = join( ' ', apply_filters( 'nav_menu_css_class', array_filter( $class_arr ), $item, $args, $depth ) );
		...
		if ( $item->menu_item_parent == 0 ) {
			...
			if ( is_array( $dropdown_settings ) ) {
				...
				if ( ! empty( $dropdown_settings['columns'] ) AND (int) $dropdown_settings['columns'] > 1 ) {
					...
					$item->columns_fill_direction = $dropdown_settings['columns_fill_direction'] ?? 'hor';
				}
