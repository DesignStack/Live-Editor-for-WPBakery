...
! function( $, _undefined ) {
	...
	$us.WPopup.prototype = {
		...
		afterShow: function() {
			...
			$us.$window
				.trigger( 'resize' )
				.trigger( 'us.wpopup.afterShow', self )
		},
		hide: function( e ) {
			const self = this;
		...
		afterHide: function() {
			...
			$us.$window
				...
				.trigger( 'us.wpopup.afterHide', self );
	...
	$.extend( $us.WPopup.prototype, {
		...
		openPostInPopup: function( e ) {
			...
			$ush.timeout( () => {
				self.$popupPostBox.addClass( 'show' );
			}, 25 );
		},
	...
	$us.$document.on( 'usPopupOpened', ( event, $popup, $popupCloser, triggerElm ) => {
		if ( $popup.hasClass( 'l-popup' ) ) {
			...
			$us.$document.on( 'keydown.usPopup', ( e ) => {
				$us.WPopup.prototype.tabFocusTrap( e, $popup, $popupCloser, triggerElm );
			} );
		}
	} );
