...
self.$controlForSchemes.toggleClass('hidden',sectionId!=='colors');if(sectionId==='buttons'||sectionId==='input_fields'){self.$controlForImportExport.removeClass('hidden')}else{self.$controlForImportExport.addClass('hidden')}
self.showSection(sectionId)},showSection:function(sectionId){const self=this;const $curItem=self.navItems.filter(`[data-id="${sectionId}"]`);$curItem.addClass('current');self.fireFieldEvent(self.$sectionContents[sectionId],'beforeShow');self.$sectionContents[sectionId].stop(!0,!1).fadeIn();self.$title.html(self.sectionTitles[sectionId]);self.fireFieldEvent(self.$sectionContents[sectionId],'afterShow');const itemPopup=$('.usof-nav-popup',$curItem);if(itemPopup.length>0){const matches=document.cookie.match(/(?:^|; )usof_visited_new_sections=([^;]*)/);const cookieValue=matches?decodeURIComponent(matches[1]):'';const visitedNewSections=(cookieValue=='')?[]:cookieValue.split(',');if(visitedNewSections.indexOf(sectionId)==-1){visitedNewSections.push(sectionId);document.cookie='usof_visited_new_sections='+visitedNewSections.join(',')}
itemPopup.remove()}
self.active=sectionId},hideSection:function(){const self=this;self.navItems.filter(`[data-id="${self.active}"]`).removeClass('current');self.fireFieldEvent(self.$sectionContents[self.active],'beforeHide');self.$sectionContents[self.active].stop(!0,!1).hide();self.$title.html('');self.fireFieldEvent(self.$sectionContents[self.active],'afterHide');self.active=null},onExportStyles:function(){const self=this;const sectionId=document.location.hash.substring(1);var name=sectionId;if(sectionId==='buttons'){name='button_styles'}
if(sectionId==='input_fields'){name='field_styles'}
const data=JSON.stringify((self.getValues()||{})[sectionId]??{});if(data){$ush.download(data,`${ name }_[%s].json`.replace('%s',window.location.host),JSON_MIME_TYPE)}},onImportStyles:function(){const self=this;const fileInput=document.createElement('input');const ERROR_INCORRECT_FILE='Incorrect file';const sectionId=document.location.hash.substring(1);const loadStyles=(e)=>{try{if(self.groups[sectionId]instanceof $usof.Group){const fileData=JSON.parse(String(e.target.result).trim());if(!Array.isArray(fileData)||fileData.length===0){throw new Error}
