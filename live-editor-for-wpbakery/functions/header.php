...
if ( ! function_exists( 'us_output_builder_elms' ) ) {
	 ...
	 */
	function us_output_builder_elms( &$settings, $state, $place, $context = 'header', $grid_object_type = 'post', $grid_has_overriding_link = FALSE ) {
		...
		foreach ( $layout[ $place ] as $elm ) {
			...
			if ( substr( $elm, 1, 7 ) == 'wrapper' ) {
				...
				us_output_builder_elms( $settings, $state, $elm, $context, $grid_object_type, $grid_has_overriding_link );
				echo $link_html ?? '';
				echo '</div>';
				...
				// Predefined context for `[us_post_title us_elm_context="..."]`
				// See issue #4926 for details.
				global $us_elm_context;
				$us_elm_context = $context;

				$values = array_merge( $defaults, $data[ $elm ] );
				$values['id'] = $elm;
				$values['atts'] = $values; // Always present in the shortcode template
