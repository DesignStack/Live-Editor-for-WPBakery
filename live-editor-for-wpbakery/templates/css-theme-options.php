...
if ( us_is_asset_used( 'buttons' ) AND $btn_styles = us_get_option( 'buttons' ) ) {
	foreach ( $btn_styles as $key => $btn_style ) {
		...
		$btn_bg_color = us_get_color( $btn_style['color_bg'] ?? '', /* Gradient */ TRUE );
		$btn_bg_color_hover = us_get_color( $btn_style['color_bg_hover'] ?? '', /* Gradient */ TRUE );
		$btn_border_color = us_get_color( $btn_style['color_border'] ?? '', /* Gradient*/ TRUE );
		$btn_border_color_hover = us_get_color( $btn_style['color_border_hover'] ?? '', /* Gradient*/ TRUE );
		...
		if ( $with_shop AND us_get_option( 'shop_secondary_btn_style' ) == $btn_style['id'] ) {
			...
			echo '.woocommerce .actions .button,';
		}
		if ( $with_shop AND us_get_option( 'shop_primary_btn_style' ) == $btn_style['id'] ) {
			echo '.woocommerce .button.alt,';
			echo '.woocommerce .button.checkout,';
		...
		if ( $btn_style['hover'] == 'circle' ) {
			...
			echo 'background:' . ( $btn_bg_color_hover ?: 'transparent' ) . ';';

		} else {
			echo 'background:' . ( $btn_bg_color ?: 'transparent' ) . ';';
		}

		// Border Color
		if ( strpos( $btn_border_color, 'grad' ) !== FALSE ) {
			echo 'border-image:' . $btn_border_color . ' 1;';
			echo 'border-color: transparent;';
		} else {
			echo 'border-color:' . ( $btn_border_color ?: 'transparent' ) . ';';
		...
		if ( $with_shop AND us_get_option( 'shop_secondary_btn_style' ) == $btn_style['id'] ) {
			echo '.woocommerce .button:before,';
			echo '.woocommerce .actions .button:before,';
		}
		if ( $with_shop AND us_get_option( 'shop_primary_btn_style' ) == $btn_style['id'] ) {
			echo '.woocommerce .button.alt:before,';
			echo '.woocommerce .button.checkout:before,';
			echo '.woocommerce .button.add_to_cart_button:before,';
		}
		...
		if ( $with_shop AND us_get_option( 'shop_secondary_btn_style' ) == $btn_style['id'] ) {
			...
			echo '.no-touch .woocommerce .actions .button:hover,';
		}
		if ( $with_shop AND us_get_option( 'shop_primary_btn_style' ) == $btn_style['id'] ) {
			echo '.no-touch .woocommerce .button.alt:hover,';
			echo '.no-touch .woocommerce .button.checkout:hover,';
		...
		if ( strpos( $btn_border_color_hover, 'grad' ) !== FALSE ) {
			echo 'border-image:' . $btn_border_color_hover . ' 1;';
			echo 'border-color: transparent;';
		} else {
			echo 'border-color:' . ( $btn_border_color_hover ?: 'transparent' ) . ';';
			echo 'border-image: none;';
		}
		...
		$bg_has_gradient = FALSE;
		if (
			strpos( $btn_bg_color, 'grad' ) !== FALSE
			OR strpos( $btn_bg_color_hover, 'grad' ) !== FALSE
		) {
			$bg_has_gradient = TRUE;
		}

		// Extra CSS for Background Hover styles OR for gradient backgrounds (cause gradients don't support transition)
		if ( $btn_style['hover'] != 'fade' OR $bg_has_gradient ) {
