...
if ( ! function_exists( 'us_output_design_css' ) ) {
	...
	function us_output_design_css() {
		...
		if (
			...
			AND get_post_status( $popup_page_template_id ) == 'publish'
		) {
		...
		foreach ( $posts as $post ) {
			...
			$post_elm_settings = get_post_meta( $post->ID, '_us_jsoncss_data', TRUE );

			if ( $post_elm_settings === '' AND function_exists( 'us_update_postmeta_for_design_css' ) ) {
				$post_elm_settings = us_update_postmeta_for_design_css( $post );
			}
			if ( ! empty( $post_elm_settings ) AND is_array( $post_elm_settings ) ) {
				foreach ( $post_elm_settings as $_elm_settings ) {
					us_append_elm_design_settings( $_elm_settings, $jsoncss_collection );
				}
			}
		}
		...
		if ( $design_options_css = us_compile_css( $jsoncss_collection ) ) {
			echo sprintf( '<style id="us-design-options-css">%s</style>', $design_options_css );
		}
	}
...
if ( ! function_exists( 'us_append_elm_design_settings' ) ) {
	/**
	 * Add Design settings of every element to the provided collection
	 *
	 * @param string $_elm_settings The jsoncss
	 * @param array $jsoncss_collection
	 * @return string Unique classname
	 */
	function us_append_elm_design_settings( $_elm_settings, &$jsoncss_collection ) {

		if ( ! empty( $_elm_settings ) AND is_string( $_elm_settings ) ) {

			$unique_class_name = us_get_unique_css_class_name( $_elm_settings );

			if ( $_elm_settings = json_decode( rawurldecode( $_elm_settings ), TRUE ) ) {

				foreach ( (array) us_get_responsive_states( /* only keys */ TRUE ) as $state ) {
					if ( $css_options = us_arr_path( $_elm_settings, $state, FALSE ) ) {
		...
		if ( preg_match( $contains_dynamic_variable_regex, $string ) ) {
			...
			$string = (string) preg_replace_callback( $contains_dynamic_variable_regex, function( $matches ) use ( $acf_format ) {
				...
				if ( $matches[0] == '{{comment_count}}' AND us_get_current_meta_type() == 'post' ) {
					...
					global $us_elm_context;
					if ( $us_elm_context ) { // See issue #4926 for details.
						return strip_tags( do_shortcode( '[us_post_title us_elm_context="' . $us_elm_context . '"]' ) );
					}
					...
					if ( is_array( $meta_value ) AND array_filter( $meta_value, 'is_scalar' ) ) {
