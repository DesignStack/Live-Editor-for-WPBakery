...
if ( ! function_exists( 'us_apply_orderby_to_list_query' ) ) {
	...
	function us_apply_orderby_to_list_query( &$query_args, $orderby_params ) {
		...
		if ( in_array( $query_args['orderby'], array( 'meta_value', 'meta_value_num', 'menu_order' ) ) ) {
...
if ( ! function_exists( 'us_list_posts_per_archive_page_save_to_post_meta' ) ) {
	...
	function us_list_posts_per_archive_page_save_to_post_meta( $post_id, $post ) {
		...
		if ( $post->post_type != 'us_content_template' ) {
			return;
		}

		if (
			strpos( $post->post_content, 'pagination="' ) !== FALSE
			AND strpos( $post->post_content, 'posts_per_archive_page="' ) !== FALSE
		) {
			$tagnames = array( 'us_post_list', 'us_product_list' );

			if ( ! preg_match_all( '/' . get_shortcode_regex( $tagnames ) . '/', $post->post_content, $matches ) ) {
				return;
			}

			foreach ( array_reverse( $matches[3] ) as $shortcode_atts ) {
				if ( strpos( $shortcode_atts, 'pagination="' ) === FALSE ) {
					continue;
				}
				if ( preg_match( '/\s?posts_per_archive_page="(\d+)"/', $shortcode_atts, $match ) ) {
					$meta_value = $match[1];
					break;
				}
			}
...
if ( ! function_exists( 'us_list_set_posts_per_archive_page' ) ) {
	...
	function us_list_set_posts_per_archive_page( $wp_query ) {
		if (
			...
			$posts_per_page = (int) get_post_meta( $content_area_id, '_us_posts_per_archive_page', TRUE );
