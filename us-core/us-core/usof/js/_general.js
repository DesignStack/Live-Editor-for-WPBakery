...
! function( $, _undefined ) {
	...
	$.extend( $usof.Form.prototype, $usof.mixins.Fieldset, {
		...
		openSection: function( sectionId ) {
			...
			if ( sectionId === 'buttons' || sectionId === 'input_fields' ) {
				self.$controlForImportExport.removeClass( 'hidden' );
			} else {
				self.$controlForImportExport.addClass( 'hidden' );
			}
		...
		onExportStyles: function() {
			...
			const data = JSON.stringify( ( self.getValues() || {} )[ sectionId ] ?? {} );
			if ( data ) {
		...
		onImportStyles: function() {
			...
			const loadStyles = ( e ) => {
				try {
					if ( self.groups[ sectionId ] instanceof $usof.Group ) {
